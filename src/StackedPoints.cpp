...
namespace mcc
{
	...
	/*
		for (std::vector<IPoint*>::iterator it = pts.begin(); it != pts.end(); ++it) {
			//std::cout << "pts degerleri" << *it << std::endl;
			printf("%d\n", *it);
		}*/
			...
			ptItor = nextPtItor, ++nextPtItor)
		{
			...
			/*if (a < 11) {
				std::cout << pt.x() <<"--"<<  pt.y() <<"--"<<pt.z() << "--" << pt.classification()<< std::endl;
				printf("%f\n", pts.operator[](a)->z());
				b++;
				a++;
			}
			std::cout << (*ptItor)->x() << std::endl;
			std::cout << (*ptItor)->y() << std::endl;
			std::cout << (*ptItor)->z() << std::endl;*/
