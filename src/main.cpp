...
shared_ptr<PointVector> readPoints(shared_ptr<IDataFormat> dataFormat, const boost::filesystem::path & filePath, XyExtent & xyExtent)
{
	...
	char* argv[])
{
	...
	try {
		...
		/*std::cout << points->operator[](0).x() << std::endl;
		std::cout << points->operator[](0).y() << std::endl;
		std::cout << points->operator[](0).z() << std::endl;*/

		duration = (std::clock() - start) / (double)CLOCKS_PER_SEC;
		std::cout << "total process time :  " << duration << " seconds" << std::endl;

/*		filesys::ifstream srcFile;

		BinaryFile::openInputFile(inputPath, srcFile);
		...
		BinaryFile::openOutputFile(outputPath, destFile);
		liblas::Writer lasWriter(destFile, header);

		while (lasReader.ReadNextPoint())
		{
			lasWriter.WritePoint(lasReader.GetPoint());
		}*/
