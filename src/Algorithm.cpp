    ...
    for (int SD = 1; SD <= 3; SD++) {
      ...
      do {
        ...
        std::cout << "SD " << SD << " - Pass " << pass << "***********************************************" << std::endl
                  << "Interpolating " << U.count() << " points:";
		
		//printf("algoritma 113.satir rastersurface üstü\n");
        boost::shared_ptr<IRasterSurface> rasterSurface = surfaceInterpolation_(U, CR[SD], tension);
		//printf("algoritma 115.satir rastersurface altý\n");
        ...
        std::cout << "Identifying non-ground points:" << std::endl;
        ...
        BOOST_FOREACH( IPoint & point, U) {
		  ...
		  //std::cout << surfaceHeight << std::endl;
        ...
        std::cout << nClassified << " points (" << percentClassifiedStr << ") classified as non-ground" << std::endl;
        if (nongroundPtsFile) {
